  ...
  included do
    def has_rich_text_content # rubocop:disable Metrics/AbcSize,Metrics/CyclomaticComplexity,Metrics/PerceivedComplexity,Naming/PredicatePrefix
      ...
      body.body.attachments.each do |attach|
        ...
        errors.add(:base, "File is too large. only files less than 10mb allowed.") if attach.byte_size > 10.megabytes

        size += attach.byte_size
      end

      errors.add(:base, "Total size of files are large. Only 15mb max for all files.") if size > 15.megabytes
    end
  end
end
